...
class LibCeresConan(ConanFile):
    ...
    def build(self):
        ...
        cxsparse_lib_path = self.deps_cpp_info["cxsparse"].lib_paths[0].replace('\\', '/')
        cxsparse_include_paths = self.deps_cpp_info["cxsparse"].include_paths[0].replace('\\', '/')
        cmake.definitions["CXSPARSE_LIBRARY_DIR_HINTS"] = cxsparse_lib_path
        cmake.definitions["CXSPARSE_INCLUDE_DIR"] = os.path.join(cxsparse_include_paths, "CXSparse")
